...
class Measurements:
    def __init__(self) -> None:
        self.measurements = [Queue() for _ in range()]
...
class RLModel(torch.nn.Module):
    ...
    def save_cpp_model(self, model_path):
        ...
        traced_script_module = torch.jit.trace(self.model, example, check_trace=False)
        traced_script_module.save(model_path)

    def save_model(self, model_path):
        torch.save({'model_state_dict': self.model.state_dict()}, 
            f"{CONFIG['weights_path']}{model_path}")
