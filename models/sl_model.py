...
def get_output_dim(model_config):
    ...
    if scoring_type == 'bin_rebuffer':
        return fill_default_key_conf(model_config, 'sl_max_bin') + 1 # bins
    if scoring_type in ['ssim_bin_rebuffer', 'bit_rate_bin_rebuffer']:
        return 2 + fill_default_key_conf(model_config, 'sl_max_bin') + 1 #ssim, ssim_change, bins


def get_sizes(model_config):   
    scoring_type = fill_default_key_conf(model_config, 'scoring_function_type')
    if scoring_type in ['bin_rebuffer', 'ssim_bin_rebuffer', 'bit_rate_bin_rebuffer']:
        return fill_default_key_conf(model_config, 'sl_bin_model_network_sizes')
...
class SLModel(NN_Model):
    ...
    def __init__(self, model_config):
       super(SLModel, self).__init__(model_config, get_config()['nn_input_size'], get_output_dim(model_config), sizes=get_sizes(model_config))
