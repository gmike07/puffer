...
def write_yaml_settings(experiments_dct):
    with open(CONFIG['yaml_path'] + 'settings.yml', 'r') as f:
...
def create_setting_yaml(generating_data='', test=False):
    ...
    if generating_data == 'randomized':
        copy_fingerprint['cc_monitoring_path'] = CONFIG['cc_monitoring_path']
        copy_fingerprint['random_cc'] = True
    ...
    if generating_data == 'fixed':
        copy_fingerprint['cc_monitoring_path'] = CONFIG['cc_monitoring_path']
        copy_fingerprint['random_cc'] = False
    ...
    if test:
        copy_fingerprint['cc_scoring_path'] = CONFIG['scoring_path']
...
def create_config(yaml_input_path, abr='', num_clients=5, test=False, eval=False, generate_data=False):
    with open(yaml_input_path + 'settings.yml', 'r') as f:
        ...
        CONFIG.update({'training': not test, 'num_clients': num_clients, 
